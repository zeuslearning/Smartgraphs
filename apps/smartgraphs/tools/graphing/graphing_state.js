...
/** @scope Smartgraphs.GRAPHING_TOOL.prototype */ {
  ...
  ON: SC.State.design({
    ...
    DISPLAY: SC.State.design({
      ...
      initialSubstate: 'CHOOSE_EQUATION',

      CHOOSE_EQUATION: SC.State.design({
        toolRoot: SC.outlet('parentState.toolRoot'),
        ...
        enterState: function () {
          ...
          if (SC.kindOf(annotation, Smartgraphs.LinearEquation)) {
            this.gotoState(this.getPath('parentState.LINEAR_EQUATION'));
          }
          else {
      ...
      LINEAR_EQUATION: SC.State.design({
        toolRoot: SC.outlet('parentState.toolRoot'),
